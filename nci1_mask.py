...
open_file.close()
model = GNN2(input_channels=37, hidden_channels=64, output_channels=2).to(device)
model.load_state_dict(torch.load('model/gcn_nci1'))
model.eval()
conv2 = GCNConv(64, 64).to(device)
conv2.load_state_dict(torch.load('model/conv2_nci1'))
conv2.eval()
conv3 = GCNConv(64, 64).to(device)
conv3.load_state_dict(torch.load('model/conv3_nci1'))
conv3.eval()
...
open_file = open('all_embed_nci1', 'wb')
pickle.dump(all_embed, open_file)
open_file.close()
open_file = open('all_final_batch_nci1', 'wb')
