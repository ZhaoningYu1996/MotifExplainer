...
model.load_state_dict(torch.load('model/gcn_mutagenicity_new', weights_only=True))
model.eval()
...
classifier.load_state_dict(torch.load('model/mlp_mutagenicity_new', weights_only=True))
classifier.eval()
                ...
                for k in node_id[i]:
                    ...
                    for j in range(edge_index.size()[1]):
                        if (edge_index[0, j]-pre_edge) in k and (edge_index[1, j]-pre_edge) in k:
                            edge_mask[j] = True
                    for j in k:
                        # print(j, pre_edge, j+pre_edge)
                        # mask[j+pre_edge] = True
                        mask[j-pre_edge-2] = True
                    count_s = 0
                    change_node = {}
