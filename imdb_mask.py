...
conv2.load_state_dict(torch.load('model/conv2_imdb'))
conv2.eval()
conv3 = GraphConv(64, 64).to(device)
conv3.load_state_dict(torch.load('model/conv3_imdb'))
conv3.eval()
...
open_file = open('all_embed_imdb', 'wb')
pickle.dump(all_embed, open_file)
open_file.close()
open_file = open('all_final_batch_imdb', 'wb')
