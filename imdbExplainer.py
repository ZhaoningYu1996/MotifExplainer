...
dataset = TUDataset(root='TUDataset', name='IMDB-BINARY', transform=OneHotDegree(135))
# Split dataset
seed = 1
import os
...
open_file = open('all_embed_imdb', 'rb')
all_embed = pickle.load(open_file)
open_file.close()
open_file = open('all_final_batch_imdb', 'rb')
...
def train(model, model2, optimizer, explainModel, classifier, loss_function):
    open_file = open('node_id_imdb', 'rb')
    ...
    if (l / collect) < min_loss:
        ...
        torch.save(explainModel.state_dict(), 'model/gcn_imdb_mean_explain')
        att_emb = explainModel.att_embedding.data.cpu()
        print(att_emb)
        open_file = open('imdb_motif_attention_emb', 'wb')
...
def test(model, explainModel, classifier):
            ...
            # if data.y == 0:
                ...
                if a == data.y:
                    ...
                    if new_x.size()[0] > 0:
                        ...
                        # correct += c
                        # count += num
    with open("att_imdb.json", "w") as outfile:
        json.dump(attVocab, outfile)
    with open("node_id_imdb.json", "w") as outfile:
    ...
    open_file = open('imdb_motif_attention', 'wb')
...
model = GNN2(input_channels=136, hidden_channels=64, output_channels=2).to(device)

model2 = GNN2(input_channels=136, hidden_channels=64, output_channels=2).to(device)
classifier = nn.Linear(64, 2).to(device)
classifier.load_state_dict(torch.load('model/mlp_imdb'))
classifier.eval()
...
model.load_state_dict(torch.load('model/gcn_imdb'))
model2.load_state_dict(torch.load('model/gcn_imdb'))
model.eval()
model2.eval()
...
explainModelTest.load_state_dict(torch.load('model/gcn_imdb_mean_explain'))
# start = time.time()
test_acc, correct_list = test(model, explainModelTest, classifier)
# end = time.time()
# print(f'Cost time: {end-start}')
open_file = open('imdb_motif_correct_list', 'wb')
